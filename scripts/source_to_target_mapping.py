...
OUTPUT_PATH = Path("docs/source_to_target_mapping.md")
...
def run_mapping():
    ...
    mappings = [
        {
            "title":       "Customers",
            "source_table":"urg_migration_duckdb.bronze_customers",
            "silver_table":"urg_migration.main_silver.silver_customers",
            "gold_table":  "urg_migration.main_gold.dim_customers",
            "columns": [
        ...
        {
            "title":       "Orders",
            "source_table":"urg_migration_duckdb.bronze_orders",
            "silver_table":"urg_migration.main_silver.silver_orders",
            "gold_table":  "urg_migration.main_gold.fact_orders",
            "columns": [
        ...
        {
            "title":       "Transactions",
            "source_table":"urg_migration_duckdb.bronze_transactions",
            "silver_table":"urg_migration.main_silver.silver_transactions",
            "gold_table":  "urg_migration.main_gold.fact_transactions",
            "columns": [
        ...
        {
            "title":       "Products",
            "source_table":"urg_migration_duckdb.bronze_products",
            "silver_table":"urg_migration.main_silver.silver_products",
            "gold_table":  "urg_migration.main_gold.dim_products",
            "columns": [
    ...
    lines = [
        ...
        f"**Generated:** {ts}",
        f"**Source:** System A (Legacy ERP — CSV exports)",
        f"**Target:** System B (DuckDB — Bronze / Silver / Gold)",
        f"**Architecture:** Bronze -> Silver -> Gold (dim/fact)",
        "",
        "---",
        "",
        "## Overview",
    ...
    for m in mappings:
        lines += [
            f"## {m['title']}",
            "",
            f"**Source (System A):** `{m['source_table']}`",
            f"**Silver:** `{m['silver_table']}`",
            f"**Gold (System B):** `{m['gold_table']}`",
            "",
            "| Source column | Source type | Target column | Target type | Transformation applied |",
            "|---------------|-------------|---------------|-------------|------------------------|",
        ]
        for src_col, src_type, tgt_col, tgt_type, transformation in m["columns"]:
            lines.append(f"| {src_col} | {src_type} | {tgt_col} | {tgt_type} | {transformation} |")
        lines += ["", "---", ""]
    ...
    print(f"Source to target mapping written to: {OUTPUT_PATH}")
    conn.close()
